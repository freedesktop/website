...
# $Source: /cvsroot/mozilla/webtools/tinderbox2/src/lib/TinderDB/Build.pm,v $ 
# $Name:  $ 
...
sub latest_status {
  ...
  my ($last_tree_data, $second2last_tree_data, $last_vc_data) = TinderDB::VC_OOo::find_last_data($tree);

  foreach $buildname (@build_names) {
    # get the starttime of the last build (to compare it with
    # the last-checkin data
    my ($rec) = $DATABASE{$tree}{$buildname}{'recs'}[0];
    my ($latest_starttime) = $rec->{'starttime'};
    my ($current_status) = $rec->{'status'};
    # when a new build was started, but is not finished yet, use the dirty state as well
    # (take the status from the previous, finished build)
    $latest_starttime = $DATABASE{$tree}{$buildname}{'recs'}[1]{'starttime'} unless BuildStatus::is_status_final($current_status);

    my ($last_status);

    if (defined($latest_starttime) && defined($last_vc_data) && ($latest_starttime < $last_vc_data)) {
        $last_status = 'dirty';
    } else {
...
sub trim_db_history {
  ...
  my ($tree_in_db);
  my (@all_trees) = ();
  @all_trees = TreeData::get_all_trees();
  my (%current_tree_build_names) = ();  # holds last index for trimming the current tree
  
  my (%build_times_hash) = (); # the hash to collect the buildtimes for all buildslaves

  # OOo-Buildslaves usually only build a tree once, so getting data for the
  # average buildtime based on one single tree is not good enough. Furthermore
  # the max-update limits is often not reached for a tree, leaving the data
  # without the average build time. Thus travel all trees to get
  # $NUM_OF_AVERAGE successful builds & set the average for every tree
  
  foreach $tree_in_db (@all_trees) {
    next if ($tree_in_db eq 'HEAD');
    my (@builds_in_tree) = ();
    @builds_in_tree = all_build_names($tree_in_db);
    # travel the builds...
    foreach $buildname (@builds_in_tree) {

      my $recs = $DATABASE{$tree_in_db}{$buildname}{'recs'};
      foreach $db_index (0 .. $#{ $recs }) {
        ...
        if ( 
             ...
             ($#{$build_times_hash{$buildname}{run_times}} < $NUM_OF_AVERAGE) &&
             1) {
          push @{$build_times_hash{$buildname}{run_times}}, $rec->{'runtime'};
        }
        
        if ( 
             (defined($rec->{'deadtime'})) &&
             ($rec->{'status'} eq 'success') && 
             ($#{$build_times_hash{$buildname}{dead_times}} < $NUM_OF_AVERAGE) &&
             1) {
          push @{$build_times_hash{$buildname}{dead_times}}, $rec->{'deadtime'};
        }
        
        if ($rec->{'starttime'} < $last_time) {
          if ($tree_in_db eq $tree) {
            $current_tree_build_names{$buildname} = $db_index;
          }
          last;
        }
      }
    }
  } # foreach tree in the database
  
  # Write the time to the HEAD pesudo-tree, this one is persistent and the
  # function that builds the popups for the HTML can take the values from
  # there...

  # first step: throw away all entries (builders may not exist anymore)
  my (@helperlist) = all_build_names('HEAD');
  foreach $buildname (@helperlist) {
    delete $DATABASE{HEAD}{$buildname};
  }
  
  @helperlist = ();
  @helperlist = keys(%build_times_hash); # we modify the hash, may affect the order

  # compute the average time & put into database
  foreach $buildname (@helperlist) {
    # We do not use the very first datapoint as it is probably
    # incomplete.
    pop @{$build_times_hash{$buildname}{run_times}};
    pop @{$build_times_hash{$buildname}{dead_times}};

    # medians are a more robust statistical estimator then the mean.
    # They will give us better answers than a typical "average"
    my $run_avg = main::median(@{$build_times_hash{$buildname}{run_times}});
    my $dead_avg = main::median(@{$build_times_hash{$buildname}{dead_times}});

    ($run_avg) &&
      ( $DATABASE{HEAD}{$buildname}{'average_buildtime'} = $run_avg );
    
    ($dead_avg) &&
      ( $DATABASE{HEAD}{$buildname}{'average_deadtime'} = $dead_avg );

  }

  # the trim DB step
  foreach $buildname (keys(%current_tree_build_names)) {
    my $last_index = $current_tree_build_names{$buildname};
...
sub status_table_header {
  ...
  foreach  $i (0 .. $#BUILD_NAMES) {
    ...
    my $avg_buildtime = $DATABASE{HEAD}{$buildname}{'average_buildtime'};
    my $avg_deadtime = $DATABASE{HEAD}{$buildname}{'average_deadtime'};
	...
	# since the current status is not final, use the one before to decide whether
        # it is dirty or not
    	$latest_starttime = $DATABASE{$tree}{$buildname}{'recs'}[1]{'starttime'};
    ...
    # if start time is earlier than last checkin to tree, it is dirty
    $latest_status = 'dirty' if (defined($latest_starttime) && ($latest_starttime < $last_vc_data) && defined($last_vc_data));
...
#                               "");
  ...
  if ( ($METADATA{$tree}{'updates_since_trim'} >
    ...
    # tim_db_history is modified to write the average times to the HEAD tree
    $self->savetree_db('HEAD');
  }
