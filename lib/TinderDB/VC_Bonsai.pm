...
# $Source: /cvsroot/mozilla/webtools/tinderbox2/src/lib/TinderDB/VC_Bonsai.pm,v $ 
# $Name:  $ 
...
sub apply_db_updates {
  ...
  # Q 29.12.2005 - Added code to keep entries on every full hour.
  # Q 04.01.2005 - Changed it to keep only every second full hour.
  if ( 
      defined($last_tree_data) && 
      defined($second2last_tree_data) &&
      ...
      ( ($main::TIME - $second2last_tree_data) < 2*$main::SECONDS_PER_HOUR ) &&
      ( ($main::TIME+29)%(2*$main::SECONDS_PER_HOUR) > 299 )
     ) {
