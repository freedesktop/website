...
class ProjectDOAPView(ProjectView):
    ...
    def get_tracker(self):
        ...
        query['portal_type'] = 'PoiLecTracker'
    ...
    def all_licenses(self):
        ...
        result = []
        if IExtensionCenterContent.providedBy(parent):
            available = parent.getField('availableLicenses').getAsGrid(parent)
            
            filteredresult = self.get_subcontent('license', 'LECRelease')
    ...
    def get_oses(self):
        ...
        filteredresult = self.get_subcontent('platform', ['LECFile', 'LECFileLink'])
        for item in filteredresult:
