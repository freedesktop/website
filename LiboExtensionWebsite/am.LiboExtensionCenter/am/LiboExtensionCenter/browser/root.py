...
class ExtensionCenterView(BrowserView):
    ...
    def project_count(self):
        ...
        """
        return len(self.catalog(portal_type = 'LECProject', path = self.context_path))
        
    def release_count(self):
        """Return number of releases
        """
        return len(self.catalog(portal_type = 'LECRelease', path = self.context_path))
    ...
    def categories(self):
        ...
        for cat in vocab.keys(): 
            if cat in filtered_values:
                ...
                rss_url = ("%s/search_rss?portal_type=LECRelease&sort_on=Date&"
