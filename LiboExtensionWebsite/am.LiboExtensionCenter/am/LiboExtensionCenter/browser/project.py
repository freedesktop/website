...
class ProjectView(BrowserView):
    ...
    def release_folder_url(self):
        ...
        ids = self.context.contentIds(
          filter = {'portal_type': 'LECReleaseFolder'})
        if ids:
    ...
    def roadmap_folder_url(self):
        ...
        ids = self.context.contentIds(
          filter = {'portal_type': 'LECImprovementProposalFolder'})
        if ids:
            return '%s/%s' % (self.context.absolute_url(), ids[0])
    ...
    def documentation_url(self):
        ...
        ids = self.context.contentIds(
          filter = {'portal_type': 'LECDocumentationFolder'})
        if ids:
    ...
    def additional_resources(self, ignore = ('LECDocumentationFolder',)):
        """
