...
class PyPIView(BrowserView):
    ...
    def _store_package(self, name, version, data):
        ...
        if not url in ('', 'UNKNOWN', None):
            ...
            if rl is None:
                ...
                try:
                    release.invokeFactory('LECFileLink', id='download')
                except Unauthorized:
                    return self.fail('Unauthorized', 401)
    ...
    def _get_package(self, normalized_name, name, version, msg=None):
        ...
        if existing_projects == []:
            ...
            sc.invokeFactory('LECProject', normalized_name)
        ...
        if releases is None:
            ...
            releases = project.injectFolder('LECReleaseFolder', 
                                            id=cid)
        ...
        if not version in versions:
            ...
            releases.invokeFactory('LECRelease', id=version)
            try:
    ...
    def file_upload(self, data=None):
        ...
        if rf is None:
            ...
            try:
                release.invokeFactory('LECFile', id=filename)
