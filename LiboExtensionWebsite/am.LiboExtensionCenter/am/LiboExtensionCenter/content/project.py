"""
$Id: LECProject.py 2011.04.23 andreasma $
"""
...
LECProjectSchema = OrderedBaseFolder.schema.copy() + Schema((
...
LECProjectSchema['subject'].widget.visible={"view":False,"edit":False}
LECProjectSchema.moveField("categories",before="allowDiscussion")
LECProjectSchema.moveField("classifiers",after="categories")
LECProjectSchema.moveField("unsupportedVersions",after="classifiers")

class LECProject(ATCTMixin, OrderedBaseFolder):
    """Project class that holds the information about the Extension Project.
    """

    implements(IProjectContent)

    archetype_name = 'Extension Project'
    immediate_view = default_view = 'lec_project_view'
    suppl_views = ()
    content_icon = 'product_icon.gif'
    schema = LECProjectSchema
    _at_rename_after_creation = True

    global_allow = False
    allowed_content_types = ('LECReleaseFolder',
                            'LECImprovementProposalFolder',
                            'PLECDocumentationFolder',)
    ...
    def initializeArchetype(self, **kwargs):
        ...
        #        not self.objectIds('LECDocumentationFolder'):
        #    self.invokeFactory('LECDocumentationFolder',
        #                       config.DOCUMENTATION_ID)
        
        if not self.objectIds('LECReleaseFolder'):
            self.invokeFactory('LECReleaseFolder', config.RELEASES_ID)
        
        # if not self.objectIds('LECImprovementProposalFolder'):
        #     self.invokeFactory('LECImprovementProposalFolder',
    ...
    def getReleaseFolder(self):
        ...
        """
        type_filter = {'portal_type' : 'LECReleaseFolder' }
    ...
    def getRoadmapFolder(self):
        ...
        folder. May return None if no roadmap folder exists.
        """
        type_filter = {'portal_type' : 'LECImprovementProposalFolder'}
        folders = self.contentValues(filter = type_filter)
        if folders:
            return folders[0]
        else:
    ...
    def getNotAddableTypes(self):
        ...
        if config.RELEASES_ID in objectIds:
            ignored.append('LECReleaseFolder')
        if config.IMPROVEMENTS_ID in  objectIds:
            ignored.append('LECImprovementProposalFolder')
        if not self.haveHelpCenter() or \
            config.DOCUMENTATION_ID in objectIds:
            ignored.append('LECDocumentationFolder')
        if config.TRACKER_ID in  objectIds:
            ignored.append('PoiLecTracker')
        return ignored
    ...
    def getVersionsVocab(self):
        ...
        """
        catalog = getToolByName(self, 'portal_catalog')
        releases = catalog.searchResults(portal_type = 'LECRelease',
                                         path = '/'.join(self.getPhysicalPath()))
    ...
    def getAvailableFeaturesAsDisplayList(self):
        ...
        search = catalog.searchResults(portal_type = 'LECImprovementProposal',
...
registerType(LECProject, config.PROJECTNAME)
