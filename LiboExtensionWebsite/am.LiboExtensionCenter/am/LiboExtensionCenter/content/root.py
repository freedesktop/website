...
from am.LiboExtensionCenter.interfaces import IExtensionCenterContent
from am.LiboExtensionCenter.storage import getFileStorageVocab
from am.LiboExtensionCenter.events.extensioncenter import \
...
LiboExtensionCenterSchema = OrderedBaseFolder.schema.copy() + Schema((
    ...
    TextField('description',
        ...
        widget=TextAreaWidget(
            ...
            description='Description for the extension center.',
    ...
    BooleanField('useClassifiers',
        ...
        widget=BooleanWidget(
            ...
            description=("Indicate whether the Extension Center uses the "
    ...
    SimpleDataGridField('availableLicenses',
        ...
        widget=SimpleDataGridWidget(
            ...
            description='Define the available licenses for extension releases. The format is Short Name | Title | URL.',
    ...
    LinesField('availableVersions',
        ...
        widget=LinesWidget(
            ...
            description='Define the vocabulary for versions that extension releases can be listed as being compatible with. The first item will be the default selection.',
    ...
    LinesField('availablePlatforms',
        ...
        widget=LinesWidget(
            ...
            description='Define the available platforms for extension files. The first line is reserved for All platforms or any equivalent labeling.',
    ...
    ReferenceField('featuredProject',
        multiValued=0,
        allowed_types=('LECProject',),
        relationship='featuredProject',
        widget=ReferenceBrowserWidget(
            label='Featured Project',
            label_msgid='label_featured_project',
            description='Featured project for the extension center '
    ...
    ReferenceField('featuredProjectRelease',
        multiValued=0,
        allowed_types=('LECRelease',),
        relationship='featuredProjectRelease',
        widget=ReferenceBrowserWidget(
            label='Featured Project Release',
            label_msgid='label_featured_project_release',
            description='Featured project release for the featured project of the extension center '
                        '(for use with the liboextensioncenter_ploneorg template).',
            description_msgid='help_featured_project',
            i18n_domain='liboextensioncenter',
        ),
...
class LiboExtensionCenter(ATCTMixin, BaseBTreeFolder):
    """A simple folderish archetype for the Extension Center."""

    implements(IExtensionCenterContent)

    content_icon = 'product_icon.gif'

    archetype_name = 'Extension Center'
    ...
    allowed_content_types = ('LECProject',)
    schema = LiboExtensionCenterSchema
    _at_rename_after_creation = True

    security = ClassSecurityInfo()
    
    typeDescription = "A container for extension projects"
    ...
    def validate_availableCategories(self, value):
        ...
        if invalid:
            return "The following short category names are in use, either " \
                   "by another category, or by a project in the extension " \
    ...
    def setProjectEvaluators(self, value, **kw):
        ...
        for id in usersToDemote:
            ...
            while 'LECEvaluator' in roles:
                roles.remove('LECEvaluator')
            if not roles:
        ...
        for id in usersToPromote:
            roles = list(self.get_local_roles_for_userid(id))
            if 'LECEvaluator' not in roles:
                roles.append('LECEvaluator')
            self.manage_setLocalRoles(id, roles)
