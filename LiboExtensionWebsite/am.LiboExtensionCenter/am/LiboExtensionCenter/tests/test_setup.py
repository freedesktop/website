...
class TestProductInstall(LECTestCase):

    def afterSetUp(self):
        self.types = {
            'LiboExtensionCenter'           : 'lec_area_workflow',
            'LECProject'                    : 'lec_package_workflow',
            'LECReleaseFolder'              : None,
            'LECRelease'                    : 'lec_release_workflow',
            'LECImprovementProposalFolder'  : None,
            'LECImprovementProposal'        : 'lec_improvementproposal_workflow',
            'LECDocumentationFolder'        : None,
            'LECFile'                       : None,
            'LECFileLink'                   : None,
        }
    ...
    def testWorkflowScriptsInstalled(self):
        ...
        release_wf = wf['lec_release_workflow'].scripts
        package_wf = wf['lec_package_workflow'].scripts
    ...
    def testNavtreePropertiesConfigured(self):
        ...
        for t in ('LiboExtensionCenter', 'LECReleaseFolder', 'LECImprovementProposalFolder', 'LECRelease'):
            self.failUnless(t in pmntq)
    
    # XXX I don't think this is a good test because
    # the dependencies are not automatically installed by LEC
