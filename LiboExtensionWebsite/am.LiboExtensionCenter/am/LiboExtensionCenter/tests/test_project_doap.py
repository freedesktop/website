...
class TestProjectDOAPView(LECTestCase):
    ...
    def test_categories_url(self):
        ...
        self.resetView()
        self.assertEqual(['http://nohost/plone/lec/by-category/cat1', 'http://nohost/plone/lec/by-category/cat3'],
          self.view.categories_url())
    
    def test_naked_description(self):
        self.proj.getField('text').set(self.proj, 'A full <b>description</b> for a LECProject')
    ...
    def test_get_oses(self):
        ...
        releases['0.9'].content_status_modify(workflow_action = 'release-beta')
        releases['0.9'].reindexObject()
        releases['0.9'].invokeFactory('LECFile', 'linux')
        releases['0.9'].linux.setPlatform('Linux')
        releases['0.9'].linux.reindexObject()
        releases['0.9'].invokeFactory('LECFileLink', 'windows')
    ...
    def test_get_oses_all_platforms(self):
        ...
        self.portal.lec.setAvailablePlatforms(['Platform independent', 'Linux', 'Windows', 'Mac OS X'])
        ...
        releases['1.0'].content_status_modify(workflow_action = 'release-final')
        releases['1.0'].reindexObject()
        releases['1.0'].invokeFactory('LECFile', 'all')
        ...
        self.assertEqual(
            [{ 'revision': '0.9', 'name': 'sid', 'date': '', 'files': [] },
             { 'revision': '1.0', 'name': 'ummagumma', 'date': '2007-01-12',
               'files': ['http://nohost/plone/lec/proj/releases/1.0/linux',
                         'http://nohost'] }],
    ...
    def test_get_tracker(self):
        ...
        if HAS_POI:
            ...
            self.proj.invokeFactory('PoiLecTracker', 'issues')
            if self.view.get_tracker() != 'http://nohost/plone/lec/proj/issues':
