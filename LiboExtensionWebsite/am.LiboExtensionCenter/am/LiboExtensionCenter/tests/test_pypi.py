...
def allowMembersToAddCenter(obj):
    perms = [p for p in obj.ac_inherited_permissions(1) if p[0] == AddExtensionCenter]
    p = perms[0]
...
class TestPyPI(LECTestCase):
    ...
    def testSubmit(self):
        ...
        # until the project is published by the LEC owner
        self.login('user2')
    ...
    def test_unexisting_classifier(self):
        ...
        classifiers = lec.getField('availableClassifiers')
        values = classifiers.get(classifiers) 
        classifiers.set(lec, values + (pg,))  
    ...
    def test_distutils_id_missing(self):
        ...
        # let's create a LEC with a project that 
        # has the same name but no distutils fixed 
        self.login('user1')
        self.portal.invokeFactory('LiboExtensionCenter', 'lec')
        lec = self.portal.lec
        lec.invokeFactory('LECProject', 'iw.dist')
        self.assertEquals(['iw.dist'], list(lec.objectIds()))
    ...
    def test_distutils_id_missing2(self):
        ...
        lec['iw.dist'].setDistutilsMainId('another.one')
    ...
    def test_filename_normalization(self):
        ...
        def createProductWithName(name):
            view = PyPIView(lec, Req({'': 'submit', 'name': name, 'version': '1.0'}))
            view.submit()
        
        # Use dotted product names when creating ids
        createProductWithName('collective.something')
        self.failUnless('collective.something' in lec.objectIds(), 'Product "collective.something" should be created as "collective.something". Existing ids are %s' % [a for a in lec.objectIds()])

        # Use lowercase
        createProductWithName('collective.BargainingAgreement')
        self.failUnless('collective.bargainingagreement' in lec.objectIds(), 'Product "collective.BargainingAgreement" should be created as "collective.bargainingagreement", using lowercase. Existing ids are %s' % [a for a in lec.objectIds()])
        
        # Don't repeat "products" for the Products.* namespace
        createProductWithName('Products.ImageRepository')
        self.failUnless('imagerepository' in  lec.objectIds(), 'Product "Products.ImageRepository" should be created as "imagerepository". Existing ids are %s' % [a for a in lec.objectIds()])
    ...
    def test_workflow_by_version(self):
        ...
        def createProductWithNameAndVersion(name, version):
            view = PyPIView(lec, Req({'': 'submit', 'name': name, 'version': version}))
            view.submit()
            return lec[name].releases[version]
