...
class RatingsMigrator(BaseInlineMigrator):
  """
  Migrate LEC Projects from the content ratings product to the twothumbs product
  """
  
  src_portal_type = src_meta_type = 'LECProject'
  ...
  def migrate_ratings(self):
      ...
      if annotations:
        if annotations.has_key('contentratings.userrating.lec_stars'):
            ratings = annotations['contentratings.userrating.lec_stars'].all_user_ratings()
...
def migrate(self):
    ...
    walker = CustomQueryWalker(portal, RatingsMigrator, query = {'portal_type':'LECProject'})
