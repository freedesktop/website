...
def v1beta7_v1beta8(self, out):
    ...
    class ReleaseStateMigrator(BaseInlineMigrator):
        src_portal_type = src_meta_type = 'LECRelease'
        ...
        def migrate_releaseState(self):
            ...
            wfdef = getattr(aq_base(wftool), 'lec_release_workflow', None)
            if maturity is not None and wfdef is not None:
                ...
                wfstate = {'action'       : None, 
                           ...
                           'review_state' : state,
                           'time'         : DateTime()}
                wftool.setStatusOf('lec_release_workflow', self.obj, wfstate)
