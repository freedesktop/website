...
sub parse_args {
  ...
  if ($tree) {
    (TreeData::tree_exists($tree)) ||
      die("tinder.cgi: tree: $tree does not exist! ".join (": ", %form)."\n");    
...
sub daemon_main {
  ...
  #print "UID: $UID - lockfile: $FileStructure::LOCK_FILE \n";
  symlink ($UID, $FileStructure::LOCK_FILE) ||
    return ;
  unless (-l $FileStructure::LOCK_FILE ) {
  	print "\n\n\nlockfile does not exist just after it should have been created!\n";
  }
  ...
  unless (-l $FileStructure::LOCK_FILE ) {
  	print "\n\n\nlockfile does not exist before regular deletion!\n";
  }
  unlink ($FileStructure::LOCK_FILE) ||
    die ("Could not remove lockfile: $FileStructure::LOCK_FILE - cause: $!\n");
